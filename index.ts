...
program
  ...
  .requiredOption('-wrbtc, --wrbtc-address <address>', 'Address of the WRBTC contract on this chain')
  .requiredOption('-ncl, --native-currency-label <string>', 'Native currency label, e.g. RBTC')
  .requiredOption(
...
program.name('npx @intrinsic-finance/deployer').version(version).parse(process.argv)
...
try {
  wrbtcAddress = getAddress(program.wrbtcAddress)
} catch (error) {
  console.error('Invalid WRBTC address', (error as Error).message)
  process.exit(1)
}
